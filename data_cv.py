...
feat_int_lvl_posterior = pandas.DataFrame({i: {int_lvl: reduce(lambda x, y: x * y,
                                                               [feature_liklihood[k][
                                                                    int_lvl] if k in feature_liklihood.columns else 1
                                                                for k in samples['features'][i]], 1)
                                               for int_lvl in intrst_lvl.keys()} for i in samples.index})

# Processing street addresses
street_address = feat_liklihood('street_address', samples, intrst_lvl)

st_addr_raw_feat = pandas.DataFrame(street_address[3])[samples.index] * feat_int_lvl_posterior
st_addr_post = street_address[0]

# processing addresses
address = feat_liklihood('display_address', samples, intrst_lvl)

addr_raw_feat = pandas.DataFrame(address[3])[samples.index] * feat_int_lvl_posterior
addr_post = address[0]
...
created_raw_feat = pandas.DataFrame(created[3])[samples.index] * feat_int_lvl_posterior
...
price_raw_feat = pandas.DataFrame(price[3])[samples.index] * feat_int_lvl_posterior
price_post = price[0]
...
build_raw_feat = pandas.DataFrame(building[3])[samples.index] * feat_int_lvl_posterior
...
mgr_raw_feat = pandas.DataFrame(manager[3])[samples.index] * feat_int_lvl_posterior
...
photos_num_raw_feat = pandas.DataFrame(photos_num[3])[samples.index] * feat_int_lvl_posterior
...
lat_raw_feat = pandas.DataFrame(latitude[3])[samples.index] * feat_int_lvl_posterior
...
lon_raw_feat = pandas.DataFrame(longitude[3])[samples.index] * pandas.DataFrame(street_address[3])[samples.index] * \
               pandas.DataFrame(address[3])[samples.index] * \
               pandas.DataFrame(price[3])[samples.index] * feat_int_lvl_posterior
...
bed_raw_feat = pandas.DataFrame(bedrooms[3])[samples.index] * feat_int_lvl_posterior
bed_post = bedrooms[0]
...
bath_raw_feat = pandas.DataFrame(bathrooms[3])[samples.index] * pandas.DataFrame(street_address[3])[samples.index] * \
                pandas.DataFrame(address[3])[samples.index] * \
                pandas.DataFrame(price[3])[samples.index] * feat_int_lvl_posterior
...
print("evaluating!")
t_feat_int_lvl_posterior = {i: {int_lvl: reduce(lambda x, y: x * y,
...
t_feat_int_lvl_posterior = pandas.DataFrame(t_feat_int_lvl_posterior).transpose()
...
t_samples_bedrooms_result = (feat_posterior(t_samples, 'bedrooms', bedrooms[1], bedrooms[2],
                                            intrst_lvl) * t_feat_int_lvl_posterior)
...
for int_lvl in intrst_lvl.keys():
    t_samples_result1[int_lvl] = {i: max([  # t_samples_saddr_result[int_lvl][i],
        # t_samples_addr_price_result[int_lvl][i],
        t_samples_addr_result[int_lvl][i],
        t_samples_created_result[int_lvl][i],
        # t_samples_addr_combo_result[int_lvl][i],
        t_samples_building_result[int_lvl][i],
...
t_samples_result1.to_csv('submission3.csv', index=False)
